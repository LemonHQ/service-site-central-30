    ...
    <section className="relative w-full h-[calc(100vh-5rem)]">
      <Carousel className="h-full w-full">
        <CarouselContent className="h-full">
          {slides.map((slide, index) => (
            <CarouselItem key={index} className="h-full">
              <div 
                className={`flex flex-col justify-center h-full w-full bg-gradient-to-br ${slide.bgColor} text-white px-4 sm:px-6 lg:px-8 relative`} 
                style={{
        ...
        <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-2 z-10">
          {slides.map((_, index) => (
            <div key={index} className="w-3 h-3 rounded-full bg-white/50" />
          ))}
        </div>
