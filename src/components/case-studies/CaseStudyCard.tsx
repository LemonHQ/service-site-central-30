    ...
    <Link to={`/case-studies/${caseStudy.id}`} className="block h-full">
      <Card className="overflow-hidden h-full transition-all duration-300 hover:shadow-[0_10px_20px_rgba(0,0,0,0.15),0_6px_6px_rgba(0,0,0,0.12)] hover:-translate-y-1 transform">
        ...
        <CardContent className="p-6">
          <div className="mb-2">
            <Badge variant="outline" className="bg-gray-100 text-gray-600 font-light">
              {caseStudy.industry}
            </Badge>
          </div>
          <h3 className="text-xl font-light mb-2 text-gray-800">{caseStudy.title}</h3>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {caseStudy.services.slice(0, 3).map((service, index) => (
              <Badge key={index} variant="secondary" className="text-xs font-light text-gray-600">
                {service}
              </Badge>
            ))}
          </div>
